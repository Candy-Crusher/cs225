...
int CBPlusTree::findmin(void)
{
    CNode* pt;
    int value;
    pt=m_Root;
    while (pt->getType()!=LEAF){
        pt=((CInternalNode*)pt)->getChild(0);
    }
    value=((CLeafNode*)pt)->getData(0);
    return value;
}

int CBPlusTree::findmax(void)
{
    CNode* pt;
    int num;
    int value;
    ...
    while (pt->getType()!=LEAF){
        num=pt->getKeyNum();
        pt=((CInternalNode*)pt)->getChild(num);
    }
    num=((CLeafNode*)pt)->getKeyNum();
    value=((CLeafNode*)pt)->getData(num-1);
    return value;
}

int CBPlusTree::find_predecessor(int key)
{
    CNode* pt;
	CLeafNode* new_pt;
	int value;
	...
	pt=m_Root;
	while (pt->getType()!=LEAF){
		for (i=0;i<pt->getKeyNum();i++){
            current=((CInternalNode*)pt)->getKeyValue(i);
			if (key<current){
				break;
			}
		}
		pt=((CInternalNode*)pt)->getChild(i);
	}
    num=pt->getKeyNum();
	for (i=0;i<num;i++){
		if (((CLeafNode*)pt)->getKeyValue(i)==key){
			break;
		}
	}
	if (i==0){
        new_pt=((CLeafNode*)pt)->getLeftSibling();
		while (new_pt->getKeyNum()==0){
			new_pt=((CLeafNode*)new_pt)->getLeftSibling();
		}
        value=new_pt->getKeyValue(new_pt->getKeyNum()-1);
	}
	else{
		value=((CLeafNode*)pt)->getKeyValue(i-1);
	}
	return value;
}

int CBPlusTree::find_successor(int key)
{
    CNode* pt;
	CLeafNode* new_pt;
	int value;
	...
	if (i==num-1){
        new_pt=((CLeafNode*)pt)->getRightSibling();
		while (new_pt->getKeyNum()==0){
			new_pt=((CLeafNode*)new_pt)->getRightSibling();
		}
        value=new_pt->getKeyValue(0);
	}
	else{
		value=((CLeafNode*)pt)->getKeyValue(i+1);
	}
